...
draw_lines()
ark = turtle.Turtle() #аркадий слишком хорош для этого мира.
ark.shape("square") #ббубубубубу
ark.color("brown")
ark.shapesize(2,2)
ark.penup() #я хочу есть
ark.speed(0)
...
def generatingObjects(iterr, shape, color, listOfObjects):
    ...
    for _ in range(iterr):
        objectt = turtle.Turtle() #гоблин+леопольд=? 
        objectt.shape(shape)
        objectt.color(color)
        objectt.penup()
...
while isPlaying:
    ...
    for obstacle in obstacles:
        ...
        if y > 390:
            ...
            obstacle.showturtle()
        if (abs(obstacle.xcor() - coor_x[curcoor]) < 5) and (abs(obstacle.ycor() - ark.ycor()) < 5):
            isPlaying = False
